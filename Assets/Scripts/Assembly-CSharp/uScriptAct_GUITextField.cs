...
public class uScriptAct_GUITextField : uScriptLogic
{
	...
	public void In([FriendlyName("Value", "The value of this text field.")] ref string Value, [FriendlyName("Position", "The position and size of the text field.")] Rect Position, [DefaultValue(50)][FriendlyName("Max Length", "The maximum allowable string length for this text field.")] int maxLength, [FriendlyName("Control Name", "Name to give to this text field GUI control.")][SocketState(false, false)][DefaultValue("")] string ControlName, [SocketState(false, false)][DefaultValue("")][FriendlyName("GUI Style", "The name of a custom GUI style to use when displaying this text field.")] string guiStyle)
	{
		m_Changed = false;
		...
		string text = ((!string.IsNullOrEmpty(guiStyle)) ? GUI.TextField(Position, Value, maxLength, GUI.skin.GetStyle(guiStyle)) : GUI.TextField(Position, Value, maxLength));
		if (Value != text)
		{
			m_Changed = true;
		}
