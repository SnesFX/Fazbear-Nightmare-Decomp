...
public class uScriptAct_ModifyListByIndexCamera : uScriptLogic
{
	...
	public void InsertIntoList(ref Camera[] VariableList, int Index, Camera[] Target, out int ListCount)
	{
		...
		{
			foreach (Camera item2 in Target)
			{
				list.Add(item2);
			}
		}
		...
		{
			foreach (Camera item3 in Target)
			{
	...
	public void RemoveFromList([FriendlyName("List", "The list to modify.")] ref Camera[] VariableList, [FriendlyName("Index", "The index number where you wish to perform the modification. If the number is larger than the elements contained in the list, the action will be performed on the largest valid index number of the list. Negative values are automatically converted to 0.")] int Index, [FriendlyName("Insert Target", "The Target variable(s) to add or remove from the list. This socket is ignored when using the Remove From List input socket.")] Camera[] Target, [FriendlyName("List Size", "The remaining number of items in the list after the modification has taken place.")] out int ListCount)
	{
		List<Camera> list = new List<Camera>(VariableList);
		if (Index < 0)
		{
